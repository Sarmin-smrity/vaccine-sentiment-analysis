...
st.set_page_config(
        page_title="Public Sentiment Analysis ",
        page_icon="Syringe",
        layout="wide",
    )
...
if selected2 == "Home":
	...
	#subheader
	# st.sidebar.subheader('Tweets Analysis')
	#radio buttons
	...
	st.markdown("###  Sentiment Count with TextBlob")
	if select == "Histogram":
	...
	st.markdown("###  Country tweets by sentiment")
	choice = st.sidebar.multiselect("Countries", ('Bangladesh', 'Bhutan', 'India', 'Nepal', 'Pakistan', 'Sri Lanka'), key = '0', default=['India'])  
	if len(choice)>0:
...
if selected2 == "Overview":
	...
	with st.spinner("Loading...Please Wait"):
		...
		# sentiment=pd.DataFrame({'textblob_sentiment':sentiment.index,'Tweets':sentiment.values})
		st.markdown("###  Sentiment Count with Vader")
		if select == "Histogram":
		...
		pivot_fig_1 = px.bar(model_a, x='Accuracy', y='Model', color = 'Library', barmode='group', height= 500)
		st.plotly_chart(pivot_fig_1)

elif selected2 == "Aspects":

	# st.set_page_config(page_title="Loading...", page_icon=":clock:", layout="wide")
	...
	with st.spinner("Loading...Please Wait"):
		...
		## Aspect topics data
		st.markdown("###  Assigning Aspect to the Topics")
		topic_aspects=pd.read_csv('data/topic_aspects.csv')
		st.write(topic_aspects.head(50))



		st.markdown("###  Aspect with Sentiment")
		data_df=pd.read_csv('data/tweets_v10.csv')
	...
	with st.spinner("Loading...Please Wait"):
		...
		st.title('Realtime Sentiment Test')

		user_input = st.text_input("Text", text)

		st.markdown("####  Sentiment test with TextBlob")
		...
		st.markdown("####  Sentiment test with VADER")
